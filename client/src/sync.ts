...
export async function syncData() {
  try {
    ...
    // Use manual filter to be robust against index issues
    const allRooms = await db.rooms.toArray();
    const unsyncedRooms = allRooms.filter(r => r.synced === 0);

    const allMessages = await db.messages.toArray();
    const unsyncedMessages = allMessages.filter(m => m.synced === 0);
